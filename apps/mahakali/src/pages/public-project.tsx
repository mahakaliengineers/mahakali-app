...
import { MapPin, Calendar, ArrowLeft, CheckCircle2, Circle } from "lucide-react";
...
interface PublicProject {
  ...
  milestones: Array<{ id: number; title: string; completed: boolean; order: number }>;
  testimonials: Array<{ id: number; authorName: string; authorRole: string | null; text: string; rating: number }>;
}
...
export default function PublicProjectPage({ projectId }: { projectId: number }) {
    ...
    <div className="min-h-screen bg-background">
      ...
      <nav className="border-b border-border bg-background/80 backdrop-blur-sm sticky top-0 z-40 px-6 py-4">
        <div className="container mx-auto flex items-center gap-3">
          <Link href="/" className="font-display font-bold text-xl tracking-tight">MAHAKALI</Link>
        </div>
      </nav>
    ...
    <div className="min-h-screen bg-background flex flex-col">
      <nav className="border-b border-border px-6 py-4">
        <div className="container mx-auto">
          <Link href="/" className="font-display font-bold text-xl">MAHAKALI</Link>
        </div>
      </nav>
      <div className="flex-1 flex flex-col items-center justify-center gap-4 text-center px-6">
        ...
        <p className="text-muted-foreground">{error || "This project doesn't exist or has been removed."}</p>
        <Link href="/" className="inline-flex items-center gap-2 bg-primary text-white px-5 py-2.5 rounded-sm font-medium text-sm hover:opacity-90 transition-opacity mt-2">
    ...
    <div className="min-h-screen bg-background">
        ...
        <div className="container mx-auto px-6 max-w-5xl">
          ...
          <div className="flex flex-col md:flex-row md:items-end justify-between gap-6">
            <div>
              ...
              <h1 className="text-3xl md:text-5xl font-display font-bold mb-4 leading-tight">{project.title}</h1>
            ...
            <div className="flex-shrink-0">
              <span className={`text-sm font-semibold px-4 py-2 rounded-sm ${STATUS_STYLE[project.status]}`}>
        ...
        <div className="grid lg:grid-cols-3 gap-10">
            ...
            <section>
              ...
              <div className="bg-muted/50 rounded-xl p-6 space-y-4">
                <div className="flex items-center justify-between">
                  ...
                  <span className="text-2xl font-bold font-display text-foreground">{project.progress}%</span>
                </div>
              ...
              <section>
                ...
                <div className="space-y-2">
                  {project.milestones.map((m, i) => (
                    <div key={m.id} className={`flex items-center gap-4 p-4 rounded-lg border ${m.completed ? "bg-green-50 border-green-200" : "bg-muted/30 border-border"}`}>
                      {m.completed
                        ? <CheckCircle2 className="w-5 h-5 text-green-600 flex-shrink-0" />
                        : <Circle className="w-5 h-5 text-muted-foreground flex-shrink-0" />
                      }
                      <span className={`text-sm font-medium ${m.completed ? "text-green-800" : "text-foreground"}`}>
                        {m.title}
                      </span>
                      {m.completed && (
              ...
              <section>
                ...
                <div className="space-y-4">
                  {project.testimonials.map(t => (
                    <div key={t.id} className="bg-muted/40 rounded-xl border border-border p-6 relative">
                      <svg viewBox="0 0 24 24" className="absolute top-5 right-5 w-8 h-8 text-muted/40" fill="currentColor">
                        <path d="M4.583 17.321C3.553 16.227 3 15 3 13.011c0-3.5 2.457-6.637 6.03-8.188l.893 1.378c-3.335 1.804-3.987 4.145-4.247 5.621.537-.278 1.24-.375 1.929-.311 1.804.167 3.226 1.648 3.226 3.489a3.5 3.5 0 01-3.5 3.5c-1.073 0-2.099-.49-2.748-1.179zm10 0C13.553 16.227 13 15 13 13.011c0-3.5 2.457-6.637 6.03-8.188l.893 1.378c-3.335 1.804-3.987 4.145-4.247 5.621.537-.278 1.24-.375 1.929-.311 1.804.167 3.226 1.648 3.226 3.489a3.5 3.5 0 01-3.5 3.5c-1.073 0-2.099-.49-2.748-1.179z" />
                      </svg>
                      <StarRow rating={t.rating} />
                      <p className="text-muted-foreground italic text-sm leading-relaxed mt-3 mb-4">"{t.text}"</p>
                      <div className="flex items-center gap-3 pt-3 border-t border-border">
                        ...
                        <div>
                          ...
                          {t.authorRole && <p className="text-xs text-primary">{t.authorRole}</p>}
                        </div>
                      </div>
                    </div>
                  ))}
          ...
          <div className="space-y-6">
            ...
            <div className="bg-muted/40 rounded-xl border border-border p-6 space-y-4 sticky top-20">
              <h3 className="font-bold text-foreground text-sm uppercase tracking-wider">Project Details</h3>
              <div className="space-y-3 text-sm">
                ...
                <InfoRow label="Start Date" value={project.startDate ? new Date(project.startDate).toLocaleDateString("en-US", { year: "numeric", month: "long", day: "numeric" }) : "—"} />
                <InfoRow label="Est. Completion" value={project.endDate ? new Date(project.endDate).toLocaleDateString("en-US", { year: "numeric", month: "long", day: "numeric" }) : "—"} />
                {totalMilestones > 0 && (
                  <InfoRow label="Milestones" value={`${completedMilestones} / ${totalMilestones} done`} />
                )}
              </div>
            ...
            <div className="bg-secondary text-white rounded-xl p-6 space-y-3">
              <h3 className="font-bold font-display">Interested in a similar project?</h3>
              <p className="text-white/70 text-sm leading-relaxed">Our engineering team is ready to discuss your vision and deliver excellence.</p>
              <a
                href="/#contact"
      ...
      <footer className="border-t border-border bg-secondary text-white/60 py-8 mt-16">
        <div className="container mx-auto px-6 text-center text-sm">
          <p>© {new Date().getFullYear()} Mahakali Engineers and Developers Pvt. Ltd. · Chabahil-07, Kathmandu, Nepal</p>
        </div>
      </footer>
    </div>
  );
