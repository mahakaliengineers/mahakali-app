...
const COST_BREAKDOWN = [
  {
    label: "Foundation & Civil Structure",
    desc: "Grey structure — excavation to roof slab",
    pct: 0.4,
    color: "bg-primary",
  },
  {
    label: "Finishing & Interior",
    desc: "Wall, plaster, woodwork, paint, tiles & marbles",
    pct: 0.35,
    color: "bg-accent",
  },
  {
    label: "Utilities & Systems",
    desc: "Electrical wiring + plumbing & sanitary",
    pct: 0.15,
    color: "bg-yellow-500",
  },
  {
    label: "Misc & Contingency",
    desc: "Design & drawing, site management, unforeseen",
    pct: 0.1,
    color: "bg-orange-500",
  },
];
...
function CalculatorTool() {
  ...
  const [landSubTab, setLandSubTab] = useState<"sqm" | "sqft" | "rapd" | "bkd">(
    "sqm",
  );
  ...
  useEffect(() => {
    ...
    const dhur = Math.round(
      (remAfterBigha - kattha * LAND_SQM_PER.kattha) / LAND_SQM_PER.dhur,
    );
    ...
    setLandFields((prev) => {
      ...
      if (!RAPD_FIELDS.includes(active)) {
        next.ropani = String(ropani);
        next.aana = String(aana);
        next.paisa = String(paisa);
        next.dam = String(dam);
      }
      if (!BKD_FIELDS.includes(active)) {
        next.bigha = String(bigha);
        next.kattha = String(kattha);
        next.dhur = String(dhur);
      }
      return next;
    });
    setLandResults([
  ...
  useEffect(() => {
    ...
    const rawDiag =
      plotUnit === "ft" ? toFt(plotDiag, plotDiagIn) : parseFloat(plotDiag);
    const a = rawA * factor,
      b = rawB * factor,
      c = rawC * factor,
      d = rawD * factor,
      diag = rawDiag * factor;
    if ([a, b, c, d, diag].some((v) => isNaN(v) || v <= 0)) {
      setPlotResult(null);
      return;
    }
    ...
    const dhur = Math.round(
      (remB2 - kattha * LAND_SQM_PER.kattha) / LAND_SQM_PER.dhur,
    );
  ...
  const inputCls =
    "bg-white/5 border border-white/10 text-white placeholder:text-white/30 focus:border-primary focus:outline-none rounded px-4 py-3 w-full text-sm";
  const selectCls =
    "bg-white/5 border border-white/10 text-white focus:border-primary focus:outline-none rounded px-4 py-3 w-full text-sm appearance-none cursor-pointer";
        ...
        <div className="grid md:grid-cols-2 gap-8">
          ...
          <div>
            ...
            {landSubTab === "sqm" && (
              <div className="space-y-3">
                ...
                <input
                  type="number"
                  min="0"
                  placeholder="e.g. 508.72"
                  value={landFields.sqm}
                  onChange={(e) => handleLandField("sqm", e.target.value)}
                  className={inputCls}
                  autoFocus
                />
              </div>
            ...
            {landSubTab === "sqft" && (
              <div className="space-y-3">
                ...
                <input
                  ...
                  value={landFields.sqft}
                  onChange={(e) => handleLandField("sqft", e.target.value)}
                  className={inputCls}
                  autoFocus
                />
              </div>
            ...
            {landSubTab === "rapd" && (
              <div className="space-y-3">
                <p className="text-xs text-white/40 uppercase tracking-wider">
                  Ropani System — values add together
                </p>
                {[
                  ...
                  <div key={f.key}>
                    ...
                    <input
                      ...
                      min="0"
                      placeholder="0"
                      value={landFields[f.key as keyof typeof landFields]}
                      onChange={(e) => handleLandField(f.key, e.target.value)}
                      className={inputCls}
                      autoFocus={i === 0}
                    />
                  </div>
            ...
            {landSubTab === "bkd" && (
              <div className="space-y-3">
                <p className="text-xs text-white/40 uppercase tracking-wider">
                  Bigha System — values add together
                </p>
                {[
            ...
            <div className="mt-4 p-3 bg-white/5 border border-white/10 rounded text-xs text-white/35 leading-relaxed">
              <span className="text-primary/60">Hilly: </span>1 Ropani = 16 Aana
              = 64 Paisa = 256 Dam = 508.72 sq.m
              <br />
              <span className="text-accent/60">Terai: </span>1 Bigha = 20 Kattha
              = 400 Dhur = 6,772.63 sq.m
            </div>
          </div>
            ...
            {!landSqm ? (
              ...
              <div className="space-y-1">
                {[
                  ...
                  {
                    label: "Ropani",
                    value: landFields.ropani,
                    hi: landSubTab === "rapd",
                  },
                  {
                    label: "Aana",
                    value: landFields.aana,
                    hi: landSubTab === "rapd",
                  },
                  {
                    label: "Paisa",
                    value: landFields.paisa,
                    hi: landSubTab === "rapd",
                  },
                  {
                    label: "Dam",
                    value: landFields.dam,
                    hi: landSubTab === "rapd",
                  },
                  {
                    label: "Bigha",
                    value: landFields.bigha,
                    hi: landSubTab === "bkd",
                  },
                  {
                    label: "Kattha",
                    value: landFields.kattha,
                    hi: landSubTab === "bkd",
                  },
                  {
                    label: "Dhur",
                    value: landFields.dhur,
                    hi: landSubTab === "bkd",
                  },
                  ...landResults.map((r) => ({
                    label: r.label,
                  ...
                  <div
                    key={i}
                    className={`flex justify-between items-center px-3 py-2 rounded border text-sm ${r.hi ? "border-primary/30 bg-primary/5" : "border-white/10 bg-white/5"}`}
                  >
                    <span className={r.hi ? "text-white/70" : "text-white/50"}>
                      {r.label}
                    </span>
                    <span
                      className={`font-mono font-bold ${r.hi ? "text-primary" : "text-white/80"}`}
                    >
                      {r.value || "0"}
                    </span>
                  </div>
        ...
        <div className="grid md:grid-cols-2 gap-8">
          <div className="space-y-4">
            ...
            <div>
              ...
              <select
                value={lengthFrom}
                onChange={(e) => setLengthFrom(Number(e.target.value))}
                className={selectCls}
              >
                {LENGTH_UNITS.map((u, i) => (
            ...
            <div>
              ...
              <select
                value={lengthTo}
                onChange={(e) => setLengthTo(Number(e.target.value))}
                className={selectCls}
              >
                {LENGTH_UNITS.map((u, i) => (
                  <option key={i} value={i} className="bg-gray-900">
          ...
          <div className="flex flex-col justify-center">
            ...
            <div className="p-8 border border-white/10 rounded bg-white/5 text-center">
              {lengthResult ? (
                <>
                  <p className="text-4xl font-mono font-black text-primary mb-2">
                    {lengthResult}
                  </p>
                  <p className="text-white/50 text-sm">
                    {LENGTH_UNITS[lengthTo].label}
                  </p>
                </>
              ) : (
            ...
            <div className="space-y-5">
              ...
              <div>
                ...
                <p className="text-xs text-white/30 mt-1">
                  Decimals allowed — e.g. 2.5 for ground + 2 full + half floor
                </p>
              </div>
              ...
              <div className="p-3 bg-white/5 border border-white/10 rounded text-xs text-white/40">
                ...
                <span className="text-white/70 font-bold">
                  Rs.{" "}
                  {COST_RATES[costBuildingType][costGrade].toLocaleString(
                    "en-IN",
                  )}{" "}
                  / sq.ft
                </span>{" "}
                · Rates are indicative estimates of Mahakali Engineers and
                Developers Pvt. Ltd. For an accurate estimate please contact
                Mahakali Engineers and Developers Pvt. Ltd.
              </div>
            </div>
            ...
            <div className="flex flex-col gap-4">
                ...
                <div className="grid grid-cols-2 gap-3">
                  <div className="p-3 bg-white/5 border border-white/10 rounded text-center">
                    ...
                    <p className="text-lg font-mono font-bold text-white">
                      {Math.round(costResult.totalSqft).toLocaleString("en-IN")}
                    </p>
                    <p className="text-xs text-white/30">
                      sq.ft ({costFloors} floor
                      {parseFloat(costFloors) > 1 ? "s" : ""})
                    </p>
                  </div>
                  <div className="p-3 bg-accent/10 border border-accent/30 rounded text-center">
                    ...
                    <p className="text-lg font-mono font-bold text-accent">
                      {Math.round(costResult.sqft).toLocaleString("en-IN")}
                    </p>
                    <p className="text-xs text-white/30">
                      sq.ft (
                      {costBuildingType === "residential" ? "70%" : "60%"} of
                      total)
                    </p>
                  </div>
                </div>
                ...
                {costResult ? (
                  <>
                    ...
                    <p className="text-white/40 text-xs mt-1">
                      ...
                      {Math.round(costResult.sqft).toLocaleString("en-IN")}{" "}
                      sq.ft built-up × Rs.{" "}
                      {costResult.perSqft.toLocaleString("en-IN")}/sq.ft
                    </p>
                    <p className="text-white/30 text-xs mt-2 leading-relaxed">
                      Includes civil structure, finishing, electrical, plumbing,
                      doors &amp; windows, and a contingency allowance.
                    </p>
                  </>
                ) : (
                  <p className="text-white/30 text-sm py-4">
                    Fill in the details to see estimate
                  </p>
                )}
              </div>
              ...
              {costResult && (
                <div className="p-3 bg-white/5 border border-white/10 rounded text-xs text-white/40 leading-relaxed">
                  ⚠ This is a rough estimate only. Actual costs vary based on
                  site conditions, material prices, design complexity, and
                  labour rates. Contact Mahakali Engineers and Developers Pvt.
                  Ltd. for a detailed quote.
                </div>
              )}
            </div>
          </div>
        ...
        <div className="grid md:grid-cols-2 gap-8">
          ...
          <div className="space-y-4">
            ...
            <div className="p-3 bg-white/5 border border-white/10 rounded text-xs text-white/50 leading-relaxed">
              <div className="font-bold text-white/70 mb-1">
                How to measure your plot:
              </div>
              Draw a diagonal across your plot to split it into 2 triangles.
              <br />
              Triangle 1 uses sides{" "}
              <span className="text-primary font-bold">A + B + Diagonal</span>.
              <br />
              Triangle 2 uses sides{" "}
              <span className="text-accent font-bold">C + D + Diagonal</span>.
            </div>
                    ...
                    <div className="flex gap-1.5">
                      <div className="relative flex-1">
                        <input
                          ...
                          placeholder={s.phFt}
                          value={s.ft}
                          onChange={(e) => s.setFt(e.target.value)}
                          className={inputCls + " pr-8"}
                        />
                        <span className="absolute right-2.5 top-1/2 -translate-y-1/2 text-xs text-white/30 pointer-events-none">
                          ft
                        </span>
                      </div>
                      <div className="relative w-20">
                        <input
                          ...
                          placeholder={s.phIn}
                          value={s.inch}
                          onChange={(e) => s.setInch(e.target.value)}
                ...
                <div className="flex gap-1.5">
                  <div className="relative flex-1">
                    <input
                      ...
                      placeholder="53"
                      value={plotDiag}
                      onChange={(e) => setPlotDiag(e.target.value)}
                      className={inputCls + " pr-8"}
                    />
                    <span className="absolute right-2.5 top-1/2 -translate-y-1/2 text-xs text-white/30 pointer-events-none">
                      ft
                  ...
                  <div className="relative w-20">
                    <input
                      ...
                      placeholder="0"
                      value={plotDiagIn}
                      onChange={(e) => setPlotDiagIn(e.target.value)}
                      className={inputCls + " pr-7"}
                    />
                    <span className="absolute right-2.5 top-1/2 -translate-y-1/2 text-xs text-white/30 pointer-events-none">
                      in
          ...
          <div className="space-y-3">
            ...
            {!plotResult ? (
              ...
              <>
                    ...
                    {[
                      {
                        label: "Ropani",
                        nepali: "रोपनी",
                        value: plotResult.ropani,
                      },
                      { label: "Aana", nepali: "आना", value: plotResult.aana },
                      {
                        label: "Paisa",
                        nepali: "पैसा",
                        value: plotResult.paisa,
                      },
                    ...
                    {[
                      {
                        label: "Bigha",
                        nepali: "बिघा",
                        value: plotResult.bigha,
                      },
                      {
                        label: "Kattha",
                        nepali: "कट्ठा",
                        value: plotResult.kattha,
                      },
                ...
                <div className="p-3 bg-white/5 border border-white/10 rounded text-xs text-white/40 leading-relaxed">
                  <span className="text-primary/60">Hilly: </span>1 Ropani = 16
                  Aana = 64 Paisa = 256 Dam = 508.72 sq.m (5,476 sq.ft)
                  <br />
                  <span className="text-amber-400/60">Tarai: </span>1 Bigha = 20
                  Kattha = 400 Dhur = 6,772.63 sq.m (72,900 sq.ft) · Calculated
...
function ContactForm() {
  ...
  const inputCls =
    "w-full rounded-none border border-white/20 bg-white/5 h-12 px-4 text-white placeholder:text-white/30 focus:outline-none focus:border-primary text-sm transition-colors";
    ...
    return (
      <div className="flex flex-col items-center justify-center py-12 text-center relative z-10">
        ...
        <p className="text-white/60 mb-2">
          Thank you! We've received your message and sent a confirmation to your
          email.
        </p>
        ...
        <button
          ...
          className="text-primary text-sm font-bold underline underline-offset-4 hover:text-primary/80 transition-colors"
        >
      ...
      <div>
        ...
        >
          ...
          {PROJECT_TYPES.map((pt) => (
            <option key={pt} value={pt} className="bg-gray-900">
              {pt}
            </option>
          ))}
        </select>
...
const STATIC_TESTIMONIALS = [
  {
    ...
    authorRole: "Director, Valley Holdings",
    text: "Mahakali Engineers delivered our commercial complex 2 months ahead of schedule. Their attention to safety and quality is unmatched in the valley.",
    rating: 5,
  },
  {
    id: -2,
    authorName: "Sita Neupane",
    authorRole: "Operations Manager, Nepal Logistics",
    text: "The structural integrity and sheer scale of the industrial warehouse they built for us proves they are leaders in heavy construction in Nepal.",
    rating: 5,
  },
  {
    id: -3,
    authorName: "Prakash Shrestha",
    authorRole: "CEO, Shrestha Developers",
    text: "From the initial soil testing to the final concrete pour, their engineering division demonstrated extreme professionalism. Highly recommended.",
    rating: 5,
  },
];
    ...
    <div className="flex gap-1">
      {[1, 2, 3, 4, 5].map((s) => (
        <svg
          key={s}
          viewBox="0 0 20 20"
          className={`w-5 h-5 ${s <= rating ? "fill-yellow-400 text-yellow-400" : "fill-gray-200 text-gray-200"}`}
        >
    ...
    <div className="relative max-w-3xl mx-auto">
      ...
      <div className="overflow-hidden">
        <AnimatePresence mode="wait" custom={dir}>
          <motion.div
            ...
            className="bg-background border border-border p-10 relative"
          >
            <Quote className="absolute top-8 right-8 h-12 w-12 text-muted/30" />
            <StarRow rating={cur.rating} />
            <p className="text-muted-foreground italic text-lg leading-relaxed mt-6 mb-8 relative z-10">
              "{cur.text}"
            </p>
            <div className="flex items-center gap-4 border-t border-border pt-6">
              <div className="w-12 h-12 bg-secondary rounded-full flex items-center justify-center text-white font-bold font-display text-lg shrink-0">
                {cur.authorName.charAt(0)}
              </div>
              <div>
                <h4 className="font-bold text-foreground">{cur.authorName}</h4>
                {cur.authorRole && (
                  <p className="text-sm text-primary">{cur.authorRole}</p>
                )}
              </div>
            </div>
          </motion.div>
      ...
      {total > 1 && (
        <div className="flex items-center justify-between mt-6">
          ...
          <button
            onClick={() => go(idx - 1, -1)}
            className="w-10 h-10 border border-border bg-background hover:bg-muted flex items-center justify-center transition-colors"
          ...
          <div className="flex gap-2">
            {items.map((_, i) => (
              <button
                ...
                className={`transition-all duration-300 rounded-full ${i === idx ? "w-6 h-2 bg-primary" : "w-2 h-2 bg-border hover:bg-muted-foreground"}`}
    ...
    <div className="min-h-screen bg-background text-foreground overflow-hidden font-sans">
      ...
      <div className="hidden lg:flex justify-between items-center bg-secondary text-white/60 text-xs px-6 py-2">
        ...
        <div className="flex items-center gap-6">
          <a
            href="mailto:mahakaliengineers885@gmail.com"
      ...
      {/* Navigation */}
      <nav
        className={`fixed left-0 right-0 z-50 transition-all duration-300 ${scrolled ? "top-0 bg-background/95 backdrop-blur-md border-b border-border shadow-sm py-4" : "top-0 lg:top-8 bg-transparent py-6"}`}
      >
        <div className="container mx-auto px-6 flex items-center justify-between">
          ...
          >
            <img
              src={logoImg}
              alt="Mahakali Engineers and Developers"
          ...
          <button
            className="lg:hidden p-2 text-white bg-primary rounded-sm"
      ...
      {mobileMenuOpen && (
        <div className="fixed inset-0 z-40 bg-background pt-24 px-6 flex flex-col gap-6 lg:hidden shadow-2xl">
          <button
            onClick={() => scrollTo("about")}
            className="text-xl font-bold text-left border-b pb-4"
          >
            About Us
          </button>
          <button
          ...
          <Button
            onClick={() => scrollTo("contact")}
            size="lg"
            className="rounded-none font-bold w-full mt-4"
          >
            Get a Quote
          </Button>
        </div>
      ...
      <section
        id="home"
        className="relative h-[100dvh] flex items-center pt-20"
      >
        ...
        <div className="container mx-auto px-6 relative z-20">
          <div className="flex items-stretch gap-6 max-w-5xl">
            ...
            >
              <motion.div
                variants={FADE_UP}
                className="inline-block bg-primary/20 backdrop-blur-sm border border-primary/50 text-white px-4 py-1.5 rounded-full mb-6 text-sm font-semibold tracking-wider uppercase"
              >
            ...
            {(() => {
              ...
              return [
                { label: "Years Experience", value: 25, suffix: "+" },
                {
                  ...
                  value:
                    s.completed > 0 ? s.completed : FALLBACK_STATS.completed,
                  suffix: "+",
                },
                {
                  label: "Active Sites",
                  value: s.active > 0 ? s.active : FALLBACK_STATS.active,
                  suffix: "",
                },
                {
              ...
              >
                ...
                <div className="text-xs md:text-sm text-white/70 font-semibold uppercase tracking-wider">
                  {stat.label}
                </div>
              </motion.div>
            ))}
          ...
          <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-0 border border-white/10">
            {[
              ...
              >
                ...
                {/* Icon */}
                <div
                  className={`w-14 h-14 ${pillar.color} flex items-center justify-center mb-6 group-hover:scale-110 transition-transform duration-300`}
                >
                  <pillar.icon
                ...
                <ul className="space-y-2">
                  {pillar.items.map((item, j) => (
                    <li
                      key={j}
                      className="flex items-center gap-2 text-sm text-white/50 group-hover:text-white/70 transition-colors"
                    >
        ...
        <div className="container mx-auto px-6">
          <div className="grid lg:grid-cols-2 gap-16 lg:gap-24 items-center">
            ...
            >
              ...
              <motion.h2
                variants={FADE_UP}
                className="text-3xl md:text-5xl font-display font-bold mb-6 leading-[1.1]"
              >
              ...
              >
                Mahakali Engineers and Developers Pvt. Ltd. is a leading force
                in Nepal's construction sector. We blend heavy engineering
                capability with meticulous project management to deliver
              ...
              <motion.div variants={FADE_UP}>
                <Button
                  className="rounded-none text-base h-14 px-8"
                  onClick={() => scrollTo("contact")}
                >
                  Partner With Us
                </Button>
        ...
        <div className="container mx-auto px-6">
          ...
          >
            ...
            <p className="text-muted-foreground text-lg">
              We bring reliability, scale, and uncompromising standards to every
              site we manage.
            </p>
          </motion.div>
            ...
            {[
              {
                ...
                desc: "Rigorous project scheduling ensures your project hits milestones and completion dates without excuses.",
              },
              {
                icon: Award,
                title: "ISO-Certified Quality",
                desc: "Our materials and methodologies comply with international quality and safety standards.",
              },
              {
                icon: Layers,
                title: "End-to-End Services",
                desc: "From soil testing to final handover, we handle the entire lifecycle of construction.",
              },
              {
                icon: Users,
                title: "Expert Engineering Team",
                desc: "Led by veteran civil and structural engineers with decades of experience in complex terrains.",
              },
              {
                icon: Headset,
                title: "24/7 Site Support",
                desc: "Continuous monitoring and immediate response teams deployed across all active project sites.",
              },
              {
                icon: ShieldCheck,
                title: "Government Registered",
                desc: "Fully licensed and recognized for handling major public infrastructure works in Nepal.",
              },
            ].map((item, i) => (
              <motion.div
                key={i}
      ...
      <section id="services" className="pt-24 pb-0 bg-background">
        <div className="container mx-auto px-6">
          ...
          >
            ...
            <h2 className="text-3xl md:text-5xl font-display font-bold mb-6">
              Dual Divisions. One Standard.
            </h2>
            <p className="text-lg text-muted-foreground">
              Operating through two specialized divisions, we cover the full
              spectrum of structural analysis and heavy construction.
            </p>
          </motion.div>
        </div>
        ...
        <div className="grid lg:grid-cols-2 min-h-[600px]">
          ...
          <motion.div
            ...
            whileInView={{ opacity: 1, x: 0 }}
            viewport={{ once: true }}
            className="bg-secondary text-secondary-foreground p-10 md:p-20 relative overflow-hidden group"
          >
                ...
                {[
                  {
                    ...
                    desc: "Detailed structural calculations, load analysis, and design of foundations, frames, and retaining systems.",
                  },
                  {
                    icon: Microscope,
                    title: "Soil & Site Investigation",
                    desc: "Geotechnical surveys, soil testing, and site feasibility studies to ensure ground-safe foundations.",
                  },
                  {
                    icon: ClipboardList,
                    title: "Project Planning & Management",
                    desc: "End-to-end project scheduling, resource allocation, cost estimation, and quality control oversight.",
                  },
                  {
                    icon: ShieldCheck,
                    title: "Civil & Infrastructure Engineering",
                    desc: "Road alignment, drainage design, bridge engineering, and public infrastructure planning.",
                  },
                ].map((item, i) => (
                  <div key={i} className="flex gap-5 group/item">
                    <div className="mt-1">
                      <item.icon
                        className="h-6 w-6 text-white/50 group-hover/item:text-primary transition-colors duration-300"
                        strokeWidth={1.5}
                      />
                    </div>
                    <div>
          ...
          <motion.div
            ...
            whileInView={{ opacity: 1, x: 0 }}
            viewport={{ once: true }}
            className="bg-muted p-10 md:p-20 relative overflow-hidden group"
          >
                ...
                {[
                  {
                    ...
                    desc: "Bungalows, apartment complexes, and gated housing projects built with premium materials and finish.",
                  },
                  {
                    icon: Building2,
                    title: "Commercial Structures",
                    desc: "Office towers, shopping centres, hotels, and mixed-use complexes delivered on time and within budget.",
                  },
                  {
                    icon: Factory,
                    title: "Industrial Construction",
                    desc: "Factories, warehouses, processing plants, and heavy-load facilities with specialized structural requirements.",
                  },
                  {
                    icon: Route,
                    title: "Roads & Bridges",
                    desc: "Highway construction, suspension and girder bridges, culverts, and government civil works across Nepal.",
                  },
                ].map((item, i) => (
                  <div key={i} className="flex gap-5 group/item">
                    <div className="mt-1">
                      <item.icon
                        className="h-6 w-6 text-secondary/50 group-hover/item:text-accent transition-colors duration-300"
                        strokeWidth={1.5}
                      />
                    </div>
                    <div>
      ...
      <section id="process" className="py-24 bg-background">
        <div className="container mx-auto px-6">
          <div className="text-center max-w-3xl mx-auto mb-16">
            ...
            <p className="text-muted-foreground text-lg">
              A systematic approach to turning blueprints into reality.
            </p>
          </div>
            ...
            {[
              {
                num: "01",
                title: "Consultation",
                desc: "Initial meeting to understand project scope, budget, and timeline.",
                icon: Users,
              },
              {
                num: "02",
                title: "Survey & Design",
                desc: "Site investigation, structural design, and approval acquisition.",
                icon: Microscope,
              },
              {
                num: "03",
                title: "Execution",
                desc: "Mobilization of resources and phase-wise construction.",
                icon: Hammer,
              },
              {
                num: "04",
                title: "Handover",
                desc: "Final quality checks, finishing, and project delivery.",
                icon: CheckCircle2,
              },
            ].map((step, i) => (
              <motion.div
          ...
          >
            <div className="max-w-2xl">
              ...
              <p className="text-lg text-white/70">
                A selection of our landmark developments shaping the future of
                Nepal.
              </p>
            </div>
            <Button
              ...
              return (
                ...
                >
                  {"id" in project && (project as any).id ? (
                    <Link
                      href={`/projects/${(project as any).id}`}
                      className="block"
                    >
                      {inner}
                    </Link>
      ...
      <section
        id="calculator"
        className="py-24 md:py-32 bg-secondary text-white"
      >
        <div className="container mx-auto px-6">
          <motion.div
            initial="hidden"
          ...
          >
            ...
            <motion.p
              variants={FADE_UP}
              className="text-white/60 max-w-2xl mx-auto text-sm"
            >
              Estimate your construction cost, calculate plot area using Heron's
              formula, convert between traditional Nepali land units (Ropani,
              Aana, Bigha, Kattha) and international measurements, and convert
              lengths — all in one place.
            </motion.p>
          </motion.div>
      ...
      <section
        id="contact"
        className="relative py-24 md:py-32 bg-background border-t border-border"
      >
        <div className="container mx-auto px-6">
          <div className="grid lg:grid-cols-2 gap-16 lg:gap-24">
            ...
            >
              ...
              >
                Contact our engineering team to discuss your next
                infrastructure, commercial, or residential project.
              </motion.p>
                ...
                >
                  ...
                  <div>
                    ...
                    <p className="text-muted-foreground">
                      ...
                      <br />
                      <a
                        href="https://mahakaliengineers.com.np"
                        target="_blank"
              ...
              <motion.div
                variants={FADE_UP}
                className="w-full h-64 overflow-hidden border border-border shadow-md"
              >
                <iframe
        ...
        <div className="container mx-auto px-6">
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-12 mb-16">
            <div className="space-y-6">
              <img
                src={logoImg}
                alt="Mahakali Engineers"
                className="h-14 w-auto object-contain filter brightness-0 invert"
              />
              <p className="text-sm leading-relaxed max-w-xs">
                Premium civil, structural, and architectural construction
                services in Nepal. Building the nation's infrastructure with
                unyielding quality since our inception.
              </p>
            </div>
          ...
          <div className="border-t border-white/10 pt-8 flex flex-col md:flex-row items-center justify-between gap-4 text-xs">
            <p>
              &copy; {new Date().getFullYear()} Mahakali Engineers and
              Developers Pvt. Ltd. All rights reserved.
            </p>
            <div className="flex items-center gap-6">
              <a
                href="https://mahakaliengineers.com.np"
