...
const STATIC_TESTIMONIALS = [
  { id: -1, authorName: "Ramesh Sharma", authorRole: "Director, Valley Holdings", text: "Mahakali Engineers delivered our commercial complex 2 months ahead of schedule. Their attention to safety and quality is unmatched in the valley.", rating: 5 },
  { id: -2, authorName: "Sita Neupane", authorRole: "Operations Manager, Nepal Logistics", text: "The structural integrity and sheer scale of the industrial warehouse they built for us proves they are leaders in heavy construction in Nepal.", rating: 5 },
  { id: -3, authorName: "Prakash Shrestha", authorRole: "CEO, Shrestha Developers", text: "From the initial soil testing to the final concrete pour, their engineering division demonstrated extreme professionalism. Highly recommended.", rating: 5 },
];
...
function StarRow({ rating }: { rating: number }) {
  return (
    <div className="flex gap-1">
      {[1, 2, 3, 4, 5].map(s => (
        <svg key={s} viewBox="0 0 20 20" className={`w-5 h-5 ${s <= rating ? "fill-yellow-400 text-yellow-400" : "fill-gray-200 text-gray-200"}`}>
...
function TestimonialsCarousel({
  ...
  testimonials: Array<{ id: number; authorName: string; authorRole: string | null; text: string; rating: number }> | null;
  idx: number;
          ...
          >
            ...
            <div className="flex items-center gap-4 border-t border-border pt-6">
              ...
              <div>
                ...
                {cur.authorRole && <p className="text-sm text-primary">{cur.authorRole}</p>}
              </div>
            </div>
          </motion.div>
        </AnimatePresence>
...
export default function Home() {
  ...
  const [testimonials, setTestimonials] = useState<Array<{ id: number; authorName: string; authorRole: string | null; text: string; rating: number }> | null>(null);
  ...
  useEffect(() => {
    ...
    fetch("/api/public/testimonials").then(r => r.json()).then(setTestimonials).catch(() => {});
  }, []);
